...
SAMPLE_IMAGE = 'image3_nobg.jpg'  # @param ['image1', 'image2', 'image3']
BG_IMAGE = 'rcf-background-1536x864.jpg'
...
def run_visualization(url):
  ...
  image=cv2.cvtColor(np.float32(np.array(resized_im)), cv2.COLOR_RGB2BGR) 
  segm_map=cv2.cvtColor(np.float32(np.array(seg_map)), cv2.COLOR_RGB2BGR) 
  bg = Image.open(BG_IMAGE)
  bg_cv2 = cv2.cvtColor(np.float32(np.array(bg)), cv2.COLOR_RGB2BGR) 
  bg_cv2 = cv2.resize(bg_cv2, (image.shape[1], image.shape[0]));
  ...
  for y in range(0, h):
      for x in range(0, w):
          if segm_map[y, x, 0] == 0:
              image[y, x] = bg_cv2[y, x]
  
  cv2.imwrite("image1_new_bg.jpg",image)
